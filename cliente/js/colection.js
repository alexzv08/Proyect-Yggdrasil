...
async function añadirCartaColeccion(element) {
    if(parseInt(element.dataset.cantidad ) == 0){
        ...
        const res = await fetch(`http://localhost:80/api/anadirAColeccion`,{
            method:"POST",
            headers:{
        ...
        element.dataset.cantidad = parseInt(element.dataset.cantidad ) + 1
        const res = await fetch(`http://localhost:80/api/updateCartaColeccion`,{
            method:"POST",
            headers:{
                "Content-Type":"application/json"
            },
...
async function quitarCartaColeccion(element) {
    if(parseInt(element.dataset.cantidad ) > 0){
        ...
        if(parseInt(element.dataset.cantidad ) > 0){
            ...
            const res = await fetch(`http://localhost:80/api/eliminarCartaColeccion`,{
                method:"POST",
                headers:{
                    "Content-Type":"application/json"
                },
